...
void recsyct_(int *uplosign, double *scale, int *m, int *n, double *a, int *lda, double *b, int *ldb, double *c, int *ldc, int *info );
...
int test()
{
	...
	// _printM(cout, C, PARAM0, PARAM1, 0, 0, PARAM0, "C");
        // wrap_dtrsyl_("N", "T", &isgn, &m, &n, L, &ldl, U, &ldu, C, &ldc, &alpha, &info);	
        recsyct_(&uplosign, &alpha, &m, &n, L, &ldl, U, &ldu, C, &ldc, &info );
	...
	do{
		...
		for(size_t i = 0; i < num_runs; i++) {
			std::vector<FLOAT> tC(initC, initC+PARAM0*PARAM1);
			recsyct_(&uplosign, &alpha, &m, &n, L, &ldl, U, &ldu, &tC[0], &ldc, &info );
		}
		end = stop_tsc(start);
		if (end > overhead)
			end -= overhead;
