 ...
 * gpr_kernel.h
 *
Decl { {u'a': Matrix[a, (20, 1), GenMatAccess], u'x': Matrix[x, (20, 1), GenMatAccess], u'lp': Scalar[lp, (1, 1), GenMatAccess], u'f': Scalar[f, (1, 1), GenMatAccess], u'K': Symmetric[K, (20, 20), LSMatAccess], u't2': Matrix[t2, (20, 1), GenMatAccess], u'L': LowerTriangular[L, (20, 20), GenMatAccess], u't0': Matrix[t0, (20, 1), GenMatAccess], u't1': Matrix[t1, (20, 1), GenMatAccess], u'var': Scalar[var, (1, 1), GenMatAccess], u'L0': LowerTriangular[L0, (20, 20), GenMatAccess], u'v': Matrix[v, (20, 1), GenMatAccess], u'y': Matrix[y, (20, 1), GenMatAccess], u'X': SquaredMatrix[X, (20, 20), GenMatAccess], u'kx': Matrix[kx, (20, 1), GenMatAccess]} }
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
static __attribute__((noinline)) void kernel(double const * X, double const * x, double * K, double * y, double * kx, double * f, double * var, double * lp)
{
  ...
  // Generating : L[20,20] = S(h(1, 20, 1), ( G(h(1, 20, 1), L[20,20],h(1, 20, 0)) Div G(h(1, 20, 0), L[20,20],h(1, 20, 0)) ),h(1, 20, 0))

  // 1-BLAC: 1x1 / 1x1
  K[20] = K[20] / K[0];

  // Generating : L[20,20] = S(h(1, 20, 1), ( G(h(1, 20, 1), K[20,20],h(1, 20, 1)) - ( G(h(1, 20, 1), L[20,20],h(1, 20, 0)) Kro T( G(h(1, 20, 1), L[20,20],h(1, 20, 0)) ) ) ),h(1, 20, 1))
  ...
  for( int fi18 = 2; fi18 <= 19; fi18++ ) {

    for( int fi43 = 0; fi43 <= fi18 - 2; fi43++ ) {

      // Generating : L[20,20] = S(h(1, 20, fi18), ( G(h(1, 20, fi18), L[20,20],h(1, 20, fi43)) Div G(h(1, 20, fi43), L[20,20],h(1, 20, fi43)) ),h(1, 20, fi43))

      // 1-BLAC: 1x1 / 1x1
      K[20*fi18 + fi43] = K[20*fi18 + fi43] / K[21*fi43];

      // Generating : L[20,20] = Sum_{j7} ( S(h(1, 20, fi18), ( G(h(1, 20, fi18), L[20,20],h(1, 20, fi43 + j7 + 1)) - ( G(h(1, 20, fi18), L[20,20],h(1, 20, fi43)) Kro T( G(h(1, 20, fi43 + j7 + 1), L[20,20],h(1, 20, fi43)) ) ) ),h(1, 20, fi43 + j7 + 1)) )

      for( int j7 = 0; j7 <= fi18 - fi43 - 2; j7++ ) {

        // 1-BLAC: (1x1)^T
        trg110[0] = K[21*fi43 + 20*j7 + 20];

        // 1-BLAC: 1x1 Kro 1x1
        kr111[0] = K[20*fi18 + fi43] * trg110[0];

        // 1-BLAC: 1x1 - 1x1
        K[20*fi18 + fi43 + j7 + 1] = K[20*fi18 + fi43 + j7 + 1] - kr111[0];
      }
    }

    // Generating : L[20,20] = S(h(1, 20, fi18), ( G(h(1, 20, fi18), L[20,20],h(1, 20, Max(0, fi18 - 1))) Div G(h(1, 20, Max(0, fi18 - 1)), L[20,20],h(1, 20, Max(0, fi18 - 1))) ),h(1, 20, Max(0, fi18 - 1)))

    // 1-BLAC: 1x1 / 1x1
    K[20*fi18 + Max(0, fi18 - 1)] = K[20*fi18 + Max(0, fi18 - 1)] / K[21*Max(0, fi18 - 1)];

    // Generating : L[20,20] = ( S(h(1, 20, fi18), ( G(h(1, 20, fi18), K[20,20],h(1, 20, fi18)) - ( G(h(1, 20, fi18), L[20,20],h(1, 20, 0)) Kro T( G(h(1, 20, fi18), L[20,20],h(1, 20, 0)) ) ) ),h(1, 20, fi18)) + Sum_{j7} ( -$(h(1, 20, fi18), ( G(h(1, 20, fi18), L[20,20],h(1, 20, j7)) Kro T( G(h(1, 20, fi18), L[20,20],h(1, 20, j7)) ) ),h(1, 20, fi18)) ) )

    // 1-BLAC: (1x1)^T
    trg130[0] = K[20*fi18];

    // 1-BLAC: 1x1 Kro 1x1
    kr131[0] = K[20*fi18] * trg130[0];

    // 1-BLAC: 1x1 - 1x1
    K[21*fi18] = K[21*fi18] - kr131[0];

    for( int j7 = 1; j7 <= fi18 - 1; j7++ ) {

      // 1-BLAC: (1x1)^T
      trg138[0] = K[20*fi18 + j7];

      // 1-BLAC: 1x1 Kro 1x1
      kr139[0] = K[20*fi18 + j7] * trg138[0];

      // 1-BLAC: 1x1 - 1x1
      K[21*fi18] = K[21*fi18] - kr139[0];
    }

    // Generating : L[20,20] = S(h(1, 20, fi18), Sqrt( G(h(1, 20, fi18), L[20,20],h(1, 20, fi18)) ),h(1, 20, fi18))

    // 1-BLAC: sqrt(1x1)
    K[21*fi18] = sqrt(K[21*fi18]);
  }


  for( int fi18 = 0; fi18 <= 18; fi18++ ) {

    // Generating : t0[20,1] = S(h(1, 20, fi18), ( G(h(1, 20, fi18), t0[20,1],h(1, 1, 0)) Div G(h(1, 20, fi18), L0[20,20],h(1, 20, fi18)) ),h(1, 1, 0))

    // 1-BLAC: 1x1 / 1x1
    y[fi18] = y[fi18] / K[21*fi18];

    // Generating : t0[20,1] = Sum_{j7} ( S(h(1, 20, fi18 + j7 + 1), ( G(h(1, 20, fi18 + j7 + 1), t0[20,1],h(1, 1, 0)) - ( G(h(1, 20, fi18 + j7 + 1), L0[20,20],h(1, 20, fi18)) Kro G(h(1, 20, fi18), t0[20,1],h(1, 1, 0)) ) ),h(1, 1, 0)) )

    for( int j7 = 0; j7 <= -fi18 + 18; j7++ ) {

      // 1-BLAC: 1x1 Kro 1x1
      kr165[0] = K[21*fi18 + 20*j7 + 20] * y[fi18];
  ...
  for( int fi18 = 0; fi18 <= 18; fi18++ ) {
    ...
    // Generating : a[20,1] = S(h(1, 20, -fi18 + 19), ( G(h(1, 20, -fi18 + 19), a[20,1],h(1, 1, 0)) Div G(h(1, 20, -fi18 + 19), L0[20,20],h(1, 20, -fi18 + 19)) ),h(1, 1, 0))

    // 1-BLAC: 1x1 / 1x1
    y[-fi18 + 19] = y[-fi18 + 19] / K[-21*fi18 + 399];

    // Generating : a[20,1] = Sum_{j7} ( S(h(1, 20, j7), ( G(h(1, 20, j7), a[20,1],h(1, 1, 0)) - ( T( G(h(1, 20, -fi18 + 19), L0[20,20],h(1, 20, j7)) ) Kro G(h(1, 20, -fi18 + 19), a[20,1],h(1, 1, 0)) ) ),h(1, 1, 0)) )

    for( int j7 = 0; j7 <= -fi18 + 18; j7++ ) {

      // 1-BLAC: (1x1)^T
      trg190[0] = K[-20*fi18 + j7 + 380];

      // 1-BLAC: 1x1 Kro 1x1
      kr193[0] = trg190[0] * y[-fi18 + 19];
  ...
  // Generating : kx[20,1] = ( Sum_{k16} ( S(h(1, 20, k16), ( G(h(1, 20, k16), X[20,20],h(1, 20, 0)) Kro G(h(1, 20, 0), x[20,1],h(1, 1, 0)) ),h(1, 1, 0)) ) + Sum_{j7} ( Sum_{k16} ( $(h(1, 20, k16), ( G(h(1, 20, k16), X[20,20],h(1, 20, j7)) Kro G(h(1, 20, j7), x[20,1],h(1, 1, 0)) ),h(1, 1, 0)) ) ) )
  ...
  for( int j7 = 1; j7 <= 19; j7++ ) {

    for( int k16 = 0; k16 <= 19; k16++ ) {

      // 1-BLAC: 1x1 Kro 1x1
      kr214[0] = X[j7 + 20*k16] * x[j7];
  ...
  // Generating : f[1,1] = ( S(h(1, 1, 0), ( T( G(h(1, 20, 0), kx[20,1],h(1, 1, 0)) ) Kro G(h(1, 20, 0), y[20,1],h(1, 1, 0)) ),h(1, 1, 0)) + Sum_{j7} ( $(h(1, 1, 0), ( T( G(h(1, 20, j7), kx[20,1],h(1, 1, 0)) ) Kro G(h(1, 20, j7), y[20,1],h(1, 1, 0)) ),h(1, 1, 0)) ) )
  ...
  for( int fi18 = 0; fi18 <= 18; fi18++ ) {
    ...
    // Generating : v[20,1] = S(h(1, 20, fi18), ( G(h(1, 20, fi18), v[20,1],h(1, 1, 0)) Div G(h(1, 20, fi18), L0[20,20],h(1, 20, fi18)) ),h(1, 1, 0))

    // 1-BLAC: 1x1 / 1x1
    kx[fi18] = kx[fi18] / K[21*fi18];

    // Generating : v[20,1] = Sum_{j7} ( S(h(1, 20, fi18 + j7 + 1), ( G(h(1, 20, fi18 + j7 + 1), v[20,1],h(1, 1, 0)) - ( G(h(1, 20, fi18 + j7 + 1), L0[20,20],h(1, 20, fi18)) Kro G(h(1, 20, fi18), v[20,1],h(1, 1, 0)) ) ),h(1, 1, 0)) )

    for( int j7 = 0; j7 <= -fi18 + 18; j7++ ) {

      // 1-BLAC: 1x1 Kro 1x1
      kr245[0] = K[21*fi18 + 20*j7 + 20] * kx[fi18];
  ...
  // Generating : var[1,1] = ( ( S(h(1, 1, 0), ( ( T( G(h(1, 20, 0), x[20,1],h(1, 1, 0)) ) Kro G(h(1, 20, 0), x[20,1],h(1, 1, 0)) ) - ( T( G(h(1, 20, 0), kx[20,1],h(1, 1, 0)) ) Kro G(h(1, 20, 0), kx[20,1],h(1, 1, 0)) ) ),h(1, 1, 0)) + Sum_{k16} ( $(h(1, 1, 0), ( T( G(h(1, 20, k16), x[20,1],h(1, 1, 0)) ) Kro G(h(1, 20, k16), x[20,1],h(1, 1, 0)) ),h(1, 1, 0)) ) ) + Sum_{j7} ( -$(h(1, 1, 0), ( T( G(h(1, 20, j7), kx[20,1],h(1, 1, 0)) ) Kro G(h(1, 20, j7), kx[20,1],h(1, 1, 0)) ),h(1, 1, 0)) ) )
  ...
  // Generating : lp[1,1] = ( S(h(1, 1, 0), ( T( G(h(1, 20, 0), y[20,1],h(1, 1, 0)) ) Kro G(h(1, 20, 0), y[20,1],h(1, 1, 0)) ),h(1, 1, 0)) + Sum_{j7} ( $(h(1, 1, 0), ( T( G(h(1, 20, j7), y[20,1],h(1, 1, 0)) ) Kro G(h(1, 20, j7), y[20,1],h(1, 1, 0)) ),h(1, 1, 0)) ) )
